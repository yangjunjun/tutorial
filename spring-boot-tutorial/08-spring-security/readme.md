...
> **本章目标**：掌握 Spring Security 6 在 Spring Boot 3 中的全新配置方式，实现完整的认证授权体系，包括 JWT 过滤器集成、RBAC 角色权限、CORS 跨域、异常处理等。
>
> **前置章节**：第 07 章（JWT 认证）
>
...
1. [Spring Security 6 核心概念](#1-spring-security-6-核心概念)
2. [Spring Boot 3 + Spring Security 6 配置方式](#2-spring-boot-3--spring-security-6-配置方式)
3. [SecurityFilterChain 配置详解](#3-securityfilterchain-配置详解)
4. [配置公开接口和受保护接口](#4-配置公开接口和受保护接口)
5. [将 JWT 过滤器集成到 Security 过滤器链](#5-将-jwt-过滤器集成到-security-过滤器链)
6. [UserDetailsService 实现](#6-userdetails-service-实现)
...
## 2. Spring Boot 3 + Spring Security 6 配置方式

### 2.1 新旧配置方式对比

**Spring Security 5.x（旧版，已废弃）**：

```java
// ❌ WebSecurityConfigurerAdapter 在 Security 6 中已被移除
@Configuration
@EnableWebSecurity
...
| 旧版（Security 5.x） | 新版（Security 6.x） |
|----------------------|---------------------|
...
## 3. SecurityFilterChain 配置详解

`SecurityFilterChain` 是 Spring Security 6 的核心 Bean，它定义了完整的安全策略：
...
> 💡 **注意**：`requestMatchers` 的顺序很重要！Spring Security 按**声明顺序**匹配，先匹配的先生效。所以要把具体规则放在前面，通用规则（`anyRequest()`）放在最后。
...
上一章我们创建了 `JwtAuthenticationFilter`，但它还不会自动生效。需要将其注册到 Security 过滤器链中，指定它在哪个过滤器之前执行。
...
### 9.4 在 SecurityFilterChain 中配置角色
...
### 11.3 在 SecurityFilterChain 中启用 CORS
...
### 13.5 在 SecurityFilterChain 中注册
...
| 文件 | 说明 |
|------|------|
| `config/SecurityConfig.java` | Security 核心配置（SecurityFilterChain、PasswordEncoder、AuthenticationManager） |
