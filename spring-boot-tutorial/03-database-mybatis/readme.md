...
> **注意**：本项目使用 `mybatis-plus-spring-boot3-starter`（注意末尾的 `3`），这是专门为 Spring Boot 3（Jakarta EE）适配的版本。如果使用旧版的 `mybatis-plus-boot-starter`，在 Spring Boot 3 中会报错。
...
| 依赖 | 说明 |
|------|------|
| `mybatis-plus-spring-boot3-starter` | MyBatis-Plus 的 Spring Boot 3 自动配置 Starter。注意是 `boot3`，不是 `boot` |
| `mysql-connector-j` | MySQL 官方 JDBC 驱动程序。`scope=runtime` 表示只在运行时使用 |

> **常见错误**：如果误用了 `mybatis-plus-boot-starter`（不带 `3`），启动时会报 `jakarta.servlet` 相关错误，因为旧版 Starter 依赖的是 `javax.servlet`（Java EE），而 Spring Boot 3 使用的是 `jakarta.servlet`（Jakarta EE）。
...
2. **引入依赖**：添加了 MyBatis-Plus Spring Boot 3 Starter 和 MySQL 驱动
