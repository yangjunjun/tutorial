...
> **本章目标**：掌握 MyBatis-Plus 3.5.1 的分页查询和条件构造器，实现文章列表的多条件筛选、排序、模糊搜索和多表联查，设计通用的分页响应 VO。
>
...
### 1.0 版本说明：MyBatis-Plus 3.5.1

本教程统一使用 **MyBatis-Plus 3.5.1**（在 pom.xml 中显式指定版本，因为 Spring Boot 2.5.12 官方不管理 MyBatis-Plus 的版本）：
...
注意 artifactId 是 `mybatis-plus-boot-starter`（Spring Boot 2 世代的标准 starter）。Spring Boot 3 世代才改用 `mybatis-plus-spring-boot3-starter`。
...
> ⚠️ **3.4.x 与 3.5.x 的写法差异**：老教程中常见的 `PaginationInterceptor`（无 Inner）是 3.4.x 之前的旧插件类，在 3.5.1 中已废弃。3.5.x 统一使用 `MybatisPlusInterceptor` + `PaginationInnerInterceptor` 组合，并且推荐在构造方法中直接传入 `DbType.MYSQL`。
...
private void validateAndNormalizeParams(ArticleQueryDTO queryDTO) {
    ...
    // 排序字段白名单（防止 SQL 注入）
    // JDK 8 写法：Arrays.asList + HashSet（JDK 9+ 才有 Set.of）
    ...
    // 排序方向白名单
    if (!ALLOWED_SORT_ORDERS.contains(
...
> 💡 **JDK 8 兼容提示**：`Set.of(...)`、`List.of(...)`、`Map.of(...)` 都是 JDK 9+ 才引入的 API。本教程使用 JDK 8，构造不可变集合请用 `new HashSet<>(Arrays.asList(...))` 或 `Collections.unmodifiableSet(...)`。
  ...
  methods: {
    async fetchArticles() {
      const res = await axios.get('/api/articles/public/page', {
        params: this.query  // axios 会自动将对象转为 URL 查询参数
      })
...
| 文件 | 说明 |
|------|------|
| `config/MybatisPlusConfig.java` | MyBatis-Plus 3.5.1 分页插件配置（`new PaginationInnerInterceptor(DbType.MYSQL)`） |
