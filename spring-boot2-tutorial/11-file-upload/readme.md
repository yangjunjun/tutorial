...
> **注意**：Spring Boot 2.5 中 `MultipartFile` 位于 `org.springframework.web.multipart` 包，
> 底层依赖的是 Servlet 规范的 `javax.servlet.http.Part`（Tomcat 9 / Servlet 4.x），
> 与 Spring Boot 3 的 jakarta 命名空间不同，但 Spring 封装层的用法完全一致。
...
文件上传成功后，需要返回文件的访问 URL、原始文件名、文件大小等信息。本章使用 **springfox 的 `@ApiModelProperty` 注解**为文档描述字段（接口文档集成见第 12 章）：
...
public class FileVO {
    ...
    @ApiModelProperty(value = "文件访问URL", example = "/uploads/avatar/2025/01/a1b2c3d4.jpg")
    private String url;
    ...
    @ApiModelProperty(value = "文件 MIME 类型", example = "image/jpeg")
...
public class FileController {
     ...
     */
    @PostMapping("/upload")
    @ApiOperation(value = "上传文件", notes = "上传文件到服务器，返回文件访问URL")
    ...
    @ApiOperation(value = "上传用户头像", notes = "仅支持 JPG/PNG/GIF，最大 2MB")
    ...
    @ApiOperation(value = "上传文章封面", notes = "仅支持 JPG/PNG，最大 5MB")
...
> Spring Boot 2.5 / Spring Security 5.5 使用 `WebSecurityConfigurerAdapter` 配置方式：
> ```java
> // SecurityConfig.java（Spring Security 5.5 风格）
> @Override
...
下一章我们将集成 **Springfox Swagger 3.0.0**，为项目自动生成在线 API 文档（Swagger UI），方便前端开发和接口调试。
