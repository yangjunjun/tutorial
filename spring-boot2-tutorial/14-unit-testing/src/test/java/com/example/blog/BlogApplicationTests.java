 ...
 * 使用 @SpringBootTest 时，2.5.x 默认的 Web 环境是 MOCK（不启动真实端口），
 * 与 Spring Boot 3 行为一致，无需额外配置。
 * </p>
 */
@SpringBootTest
@ActiveProfiles("test")
...
class BlogApplicationTests {
    ...
    void shouldUseH2DatabaseInTestProfile() {
        String[] activeProfiles = applicationContext.getEnvironment()
            .getActiveProfiles();
        assertTrue(activeProfiles.length > 0, "应至少激活一个 Profile");
        assertEquals("test", activeProfiles[0], "激活的 Profile 应为 test");
