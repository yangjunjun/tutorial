 ...
 * 典型的开发环境跨域场景：
 * <pre>
 * 前端（Vue）：http://localhost:8081
 * 后端（Spring Boot）：http://localhost:8080
 * → 协议和端口都不同，属于跨域
 * </pre>
 * <p>
 * <strong>Spring Security 5.5 中的 CORS 配置</strong>
 * <p>
 * CORS 配置需要在两个地方生效：
 * 1. CorsConfigurationSource Bean（提供 CORS 规则）
 * 2. SecurityConfig 中 http.cors()（让 Security 过滤器链感知 CORS）
 * <p>
 * 如果只在 MVC 层配置了 CORS 而 Security 层没有启用，
 * 那么 Security 的过滤器（如 JWT 过滤器）可能在 CORS 预检请求（OPTIONS）时
 * 就返回 401/403，导致跨域请求失败。
 * <p>
 * 本章将 CorsConfigurationSource 定义为独立的 Bean，
 * 供 SecurityConfig 中 http.cors().configurationSource(...) 直接引用。
 *
 * @author tutorial
 */
@Configuration
public class CorsConfig {
     ...
     * 配置 CORS 规则源
     * <p>
     * {@link CorsConfigurationSource} 是 CORS 规则的"数据源"，
     * Spring Security 的 CorsFilter 会从这里读取每个请求的 CORS 配置。
     * <p>
     * 关键配置项：
