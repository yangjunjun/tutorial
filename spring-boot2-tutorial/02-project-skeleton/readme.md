...
> **JDK 8 用户须知**：Lombok 是通过注解处理器在编译期工作的，JDK 8 完全支持上述所有注解。唯一需要注意的是：较老版本的 Lombok 在升级 JDK 后可能报错，请确保 Lombok 版本在 1.18.x 以上（Spring Boot 2.5.12 的 BOM 默认管理的是 1.18.22，无需手动指定）。
...
- 使用 `LocalDateTime` 而非 `Date`，因为 `LocalDateTime` 是 Java 8+ 的线程安全时间类（JDK 8 引入的 `java.time` 包正是本教程技术栈的基础之一）
