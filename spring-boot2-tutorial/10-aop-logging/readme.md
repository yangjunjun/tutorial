...
该 starter 自动引入了 `spring-aop` 和 `aspectjweaver`（Spring Boot 2.5.12 对应 Spring Framework 5.3.x，内嵌 aspectjweaver 1.9.x），无需额外配置。Spring 5.3 的 AspectJ 注解风格（`@Aspect`、`@Pointcut`、`@Around` 等）与之前版本一致，可以直接使用。
...
> **提示**：Spring Boot 2.5.12 运行在 JDK 8 上，`@Around` 环绕通知、`ProceedingJoinPoint` 等 API 与 JDK 8 完全兼容，本章代码全部使用 JDK 8 语法。
...
首先创建操作日志表。执行以下 SQL（完整文件见 `sql/add_operation_log.sql`）：
...
public class OperationLogAspect {
    ...
    public Object around(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        // 获取当前 HTTP 请求（javax.servlet，注意不是 jakarta！）
...
> **JDK 8 / Spring Boot 2.5 适配要点**：
> - `HttpServletRequest` 的 import 是 `javax.servlet.http.HttpServletRequest`（Tomcat 9 / Servlet 4.x），Spring Boot 3 的 `jakarta.servlet.http.HttpServletRequest` 在本教程中不可用
> - `StringUtils.hasText()` 来自 `org.springframework.util`（Spring 5.3），用法一致
...
> **注意**：使用 `@Async` 需要在配置类或启动类上添加 `@EnableAsync` 注解。
...
public class LogController {
    ...
    public Result<Page<OperationLogEntity>> page(
        ...
        wrapper.like(StringUtils.hasText(module),
               ...
               .like(StringUtils.hasText(operatorName),
...
| 操作日志存储         | 数据库表设计 + MyBatis-Plus 3.5.1 实体映射                 |
| IP 地址获取          | 处理反向代理场景下的真实 IP 获取                           |
| 异步保存             | `@Async` 异步写入日志，不阻塞业务方法                      |
| 接口耗时统计         | 使用 `execution` 切点拦截所有 Controller 方法               |
| 日志查询             | 分页查询、条件筛选操作日志                                 |

> **注意**：Spring Boot 2.5.12 基于 Servlet 容器（Tomcat 9），Servlet 相关 API 的包名是 `javax.servlet.*` 而不是 `jakarta.servlet.*`。如果从 Spring Boot 3 教程迁移代码，请务必替换所有 `jakarta` 导入。
