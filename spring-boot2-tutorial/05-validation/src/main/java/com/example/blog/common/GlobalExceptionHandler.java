 ...
 *   <li>{@link MethodArgumentTypeMismatchException} — 路径参数/请求参数类型不匹配</li>
 *   <li>{@link NoHandlerFoundException} — 请求路径不存在（需开启 throw-exception-if-no-handler-found）</li>
 * </ul>
 *
 * <h3>与 Spring Boot 3 的差异：</h3>
 * <p>
 * Spring Boot 3.2 引入了 {@code NoResourceFoundException} 用于表示静态资源不存在；
 * Spring Boot 2.5（Spring 5.3）中没有该异常，静态资源不存在时默认直接返回 404 错误页，
 * 不进入 @ExceptionHandler 流程，因此本版本无需（也无法）处理该异常。
 * </p>
 *
 * <h3>处理顺序原则：</h3>
 * <p>
...
public class GlobalExceptionHandler {
     ...
     * 校验失败会抛出 {@link ConstraintViolationException}（javax.validation 包）。
     * </p>
     * <p>
     * 示例：
     * <pre>{@code
    ...
    /**
     * 处理路径参数 / 请求参数类型不匹配（Spring 5.3 中常用的 400 类异常）
     * <p>
     * 例如接口声明 {@code @PathVariable Long id}，但前端传了字符串 {@code /api/articles/abc}，
     * 类型转换失败会抛出此异常。
     * </p>
     */
     ...
     * 处理请求路径不存在
     * <p>
     * 在 Spring Boot 2.5 中，默认情况下访问不存在的路径由 DispatcherServlet 返回
     * 404 错误页；只有配置 {@code spring.mvc.throw-exception-if-no-handler-found=true}
     * 且 {@code spring.resources.add-mappings=false} 后，才会抛出
     * {@link NoHandlerFoundException} 并进入本处理器。
     * </p>
     */
    ...
    public Result<Void> handleNoHandlerFoundException(NoHandlerFoundException e,
                                                        HttpServletRequest request) {
        log.warn("请求路径不存在 [URI={}]: {}", request.getRequestURI(), e.getMessage());
        return Result.fail(ResultCode.NOT_FOUND, "请求的接口不存在: " + e.getRequestURL());
    }
