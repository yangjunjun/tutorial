...
> **个人博客系统** — Spring Boot 2.5 实战教程（JDK 8）
>
...
<!-- 连接池（推荐 Lettuce，Spring Boot 2.x 默认使用） -->
<!-- Lettuce 已内置，无需额外依赖 -->
...
> **版本说明（Spring Boot 2.5.12）**：
>
> - `spring-boot-starter-data-redis` 无需指定版本号，由 2.5.12 父 POM 统一管理，
>   对应 **Spring Data Redis 2.5.x**、**Lettuce 6.1.x**。
> - 如需使用连接池（`lettuce.pool.*` 配置生效），必须额外引入 `commons-pool2`，
>   版本同样由父 POM 管理。
...
| 客户端       | 说明                                    | 推荐程度 |
| ------------ | --------------------------------------- | -------- |
| Lettuce      | Spring Boot 2.x 起默认客户端，基于 Netty，线程安全 | 推荐     |
...
```yaml
spring:
  redis:                     # 注意：Spring Boot 2.5.x 的配置前缀是 spring.redis
...
> ⚠️ **与 Spring Boot 3 的配置差异**：
>
> | 配置项 | Spring Boot 2.5.x（本教程） | Spring Boot 3.x |
> | ------ | --------------------------- | --------------- |
> | 前缀   | `spring.redis.*`            | `spring.data.redis.*` |
> | 示例   | `spring.redis.host`         | `spring.data.redis.host` |
>
> 如果你在网上看到 `spring.data.redis.host` 的写法，那是 Spring Boot 3 的配置，
> 在 2.5.12 下**不会生效**（且不报错），这是最容易踩的坑之一。
...
完整实现还包含 `CacheManager` 配置（用于 Spring Cache 注解）以及一个带
`JavaTimeModule` 的自定义 `ObjectMapper`（支持 `LocalDateTime` 等 JSR-310 时间类型），
详见 `RedisConfig.java` 源码。
...
在配置类（本章为 `RedisConfig`）或启动类上添加 `@EnableCaching`：
...
> ⚠️ **JDK 8 API 兼容说明**：上面互斥锁代码中，`setIfAbsent(key, value, timeout, unit)`
> 是 Spring Data Redis 2.1+ 提供的重载方法，在 2.5.x 中可直接使用。
> 也可以用 `Duration` 版本：`setIfAbsent(key, value, Duration.ofSeconds(10))`。
...
# 查看 Redis 中的排行数据
redis-cli ZREVRANGE article:ranking:views 0 -1 WITHSCORES
# 1) "1"      2) "10"
# 3) "2"      4) "5"
```
...
| Spring Boot 集成     | spring-boot-starter-data-redis（2.5.x 默认 Lettuce 6.1） |
| 序列化配置           | GenericJackson2JsonRedisSerializer 替代 JDK 序列化        |
| 配置前缀             | **Spring Boot 2.5.x 使用 `spring.redis.*`**（非 spring.data.redis） |
...
下一章我们将学习**单元测试与集成测试**，为博客系统建立完善的测试体系，保证代码质量。
