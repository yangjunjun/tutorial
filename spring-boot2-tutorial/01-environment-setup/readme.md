...
> **本章目标**：从零开始搭建 Spring Boot 2.5 开发环境，创建个人博客系统后端项目，并运行第一个 Hello World 接口。
...
> **为什么选择 Spring Boot 2.5？** 国内大量企业存量项目仍运行在 JDK 8 + Spring Boot 2.x 技术栈上。2.5.12 是 2.5.x 分支的最后一个补丁版本，修复了该分支的全部已知问题，是维护和学习 2.x 世代项目的理想选择。掌握了这套体系，再迁移到 Spring Boot 3 也只是"javax → jakarta"等少量改动。
...
Spring Boot 2.5 要求 JDK 8 及以上版本（推荐使用 JDK 8 或 11，本教程统一使用 JDK 8）。推荐使用以下任一发行版：

- **Eclipse Temurin (Adoptium) 8**：https://adoptium.net/temurin/releases/?version=8
- **Oracle JDK 8**：https://www.oracle.com/java/technologies/javase/javase8u211-later-archive-downloads.html（需注册 Oracle 账号）
- **Amazon Corretto 8**：https://aws.amazon.com/corretto/
- **Zulu OpenJDK 8**：https://www.azul.com/downloads/?version=java-8-lts

> **小贴士**：推荐使用 [SDKMAN!](https://sdkman.io/)（macOS/Linux）或 [Scoop](https://scoop.sh/)（Windows）管理多版本 JDK，可以在 JDK 8 / 11 / 17 之间随时切换：
>
> ```bash
...
```bash
java -version
# 预期输出包含 "1.8.0_xxx"（JDK 8 的版本号格式是 1.8 而非 8）
...
1. 新建系统变量 `JAVA_HOME`，值为 JDK 安装路径，如 `C:\Program Files\Java\jdk1.8.0_412`
...
```bash
export JAVA_HOME=/usr/lib/jvm/temurin-8-jdk-amd64   # 根据实际安装路径修改
export PATH=$JAVA_HOME/bin:$PATH
```

### 1.2.2 安装 Maven

Maven 是 Java 生态中最常用的项目构建和依赖管理工具。Spring Boot 2.5 对 Maven 的最低要求是 3.5+，推荐使用 3.6 ~ 3.8 版本。

下载地址：https://maven.apache.org/download.cgi

下载 Binary zip archive，解压到任意目录（如 `C:\apache-maven-3.8.8`），然后配置环境变量：
...
# 注意确认 "Java version" 一行显示的是 1.8.x，
# 如果显示的是其他版本，请检查 JAVA_HOME 是否指向 JDK 8
```
...
> **IDEA 与 JDK 8**：在 `File → Project Structure → Project` 中将 `Project SDK` 设置为 JDK 8，`Language level` 选择 `8 - Lambdas, type annotations etc.`。IDEA 本身运行在什么 JDK 上无所谓，只要项目编译用 JDK 8 即可。
...
> **重要说明**：Spring Boot 2.5.12 已于 2023 年 8 月停止 OSS 维护，目前 Spring Initializr 默认版本列表中只提供 3.x 系列。你需要：
>
> 1. 网页版：点击版本选择框中的 **"SHOW ALL"**（或直接修改 `spring boot` 版本号输入框），如果列表中没有 2.5.12，可以先任选一个版本生成项目，然后**手动修改 `pom.xml` 中的 `<version>` 为 `2.5.12`**；
> 2. IDEA 版：在 New Project 向导的 Server URL 后可点击齿轮图标切换到 `https://start.aliyun.com/`（阿里云 Initializr），其中仍然提供 Spring Boot 2.5.x 的选项。
>
> 无论采用哪种方式，最终都以 `pom.xml` 中的版本号为准。
...
2. 左侧选择 **Spring Initializr**（若列表中没有 2.5.x 版本可选，参考上方"重要说明"切换 Server URL 或先生成后手动改版本）
3. 填写项目信息（与上表一致）
...
1. **`spring-boot-starter-parent`**：这是 Spring Boot 的父 POM，它统一管理所有 Spring Boot 官方依赖的版本号，我们在引入依赖时不需要手动指定版本。2.5.12 的 BOM 中管理的常用依赖包括：内嵌 Tomcat 9.0.x（基于 `javax.servlet`）、Spring Framework 5.3.x、Jackson 2.12.x、MySQL 驱动 `mysql:mysql-connector-java` 8.0.x 等。

2. **`<java.version>1.8</java.version>`**：这个属性会被 parent 传递给 Maven 编译插件的 `maven.compiler.source` / `maven.compiler.target`。JDK 8 对应的版本号写法是 `1.8`（注意 Spring Boot 3 世代写的是 `17`）。

3. **`spring-boot-starter-web`**：这是一个 Starter（启动器），它会自动引入 Spring MVC、内嵌 Tomcat、Jackson（JSON 处理）等一系列 Web 开发所需的依赖。

4. **`lombok`**：标记为 `<optional>true</optional>`，表示这个依赖不会被传递到依赖此项目的其他项目中。同时在打包插件中排除了 Lombok，因为 Lombok 只在编译期使用。

5. **`spring-boot-maven-plugin`**：这个插件可以将项目打包成一个可执行的 "Fat JAR"（包含所有依赖的 JAR 文件）。

> **javax 与 jakarta**：Spring Boot 2.x 整个生态（Servlet、Validation、Annotation 等）都基于 `javax.*` 包名空间；到了 Spring Boot 3 世代才切换为 `jakarta.*`。本教程全部使用 `javax.*`，后续章节在参数校验（`javax.validation.constraints`）和 Servlet（`javax.servlet`）场景中请务必留意这一点。
...
| 启动失败 | JDK 版本不匹配 | 确认使用 JDK 8+，检查 `java -version` |
| `UnsupportedClassVersionError` | 编译版本高于运行版本 | 确认运行时 JDK 版本 ≥ 编译版本，本教程统一用 JDK 8 |
| `Invalid bound type` / 依赖冲突 | Spring Initializr 默认生成 3.x 版本 | 手动将 parent 版本改为 2.5.12，Java 版本改为 8 |
...
1. **环境准备**：安装了 JDK 8、Maven、IntelliJ IDEA 和 MySQL 8
2. **创建项目**：通过 Spring Initializr 生成了 Spring Boot 2.5.12 项目骨架（注意 2.5.12 可能需要手动修改版本号）
