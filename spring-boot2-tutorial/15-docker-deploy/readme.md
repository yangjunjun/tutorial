...
```dockerfile
# 使用 JDK 8 作为基础镜像（Spring Boot 2.5.12 最低支持 JDK 8）
...
| 镜像 | 说明 | 大小（约） |
|------|------|-----------|
| `eclipse-temurin:8-jre` | Temurin（AdoptOpenJDK 延续）官方 JRE 8，本教程选用 | 230MB |
| `openjdk:8-jre-slim` | OpenJDK 官方精简版（已停止更新，仍可使用） | 200MB |
| `amazoncorretto:8` | Amazon Corretto JDK 8 | 320MB |
| `maven:3.8-openjdk-8` | Maven + JDK 8 构建镜像（仅构建阶段使用） | 500MB |

> ⚠️ **JDK 8 注意事项**：
> 1. JDK 8 没有官方 Alpine 镜像（早期的 `openjdk:8-jre-alpine` 已废弃且缺少 glibc），
>    不要在 JDK 8 项目中使用它。
> 2. JDK 8 默认的垃圾收集器是 Parallel GC。G1GC 在 JDK 8 中可用（`-XX:+UseG1GC`），
>    但没有 JDK 17 中那么成熟。小内存容器建议保持 Parallel GC 或按需切换。
> 3. 容器内存感知（`-XX:+UseContainerSupport`）从 JDK 8u191 起默认开启，
>    建议使用较新的 8u2xx+ JRE 镜像，让 JVM 能感知容器的内存限制。
...
### 15.2.3 Spring Boot 分层 JAR（layertools）说明

从 **Spring Boot 2.3** 起官方支持分层 JAR（layertools），2.5.12 完全支持。分层后 Docker 只需要重新上传变化的那一层，显著加速镜像构建与推送：

```bash
# 1. 从分层 JAR 中解出各层
java -Djarmode=layertools -jar app.jar extract --destination extracted

# 2. 在 Dockerfile 中分层 COPY
...
> 本教程为保持 Dockerfile 简单易读，采用直接 `COPY *.jar` 的方式。
> 如果你的镜像较大、构建频繁，推荐切换到分层模式。
...
services:
  ...
  blog-app:
    ...
    environment:
      ...
      # Spring Boot 2.5.x 的 Redis 环境变量前缀是 SPRING_REDIS_*
...
> ⚠️ **Spring Boot 2.5.x 环境变量速查（易错点）**：
>
> | 配置项 | Spring Boot 2.5.x（本教程） | Spring Boot 3.x |
> | ------ | --------------------------- | --------------- |
> | Redis 主机 | `SPRING_REDIS_HOST` | `SPRING_DATA_REDIS_HOST` |
> | Redis 端口 | `SPRING_REDIS_PORT` | `SPRING_DATA_REDIS_PORT` |
> | Redis 密码 | `SPRING_REDIS_PASSWORD` | `SPRING_DATA_REDIS_PASSWORD` |
>
> 如果你从网上复制了 Spring Boot 3 的 compose 文件，`SPRING_DATA_REDIS_HOST`
> 在 2.5.12 中不会生效，应用会尝试连接 localhost 导致启动失败。
...
脚本包含：用户表、文章表、分类表、标签表、文章标签关联表、评论表、操作日志表，
以及初始管理员（admin / admin123）、示例分类、标签、文章和评论数据。关键片段：
...
> ⚠️ **MySQL 8 认证插件说明**：MySQL 8.0 默认使用 `caching_sha2_password` 认证插件，
> 旧版 JDBC 驱动（5.x）连接会报 `Unknown auth plugin` 错误。本教程使用 8.0.x 驱动没有问题，
> 但为兼容更多客户端，docker-compose 中我们仍显式配置了
> `--default-authentication-plugin=mysql_native_password`。
...
services:
  ...
  blog-app:
    ...
    environment:
      - SPRING_PROFILES_ACTIVE=prod
      # 生产环境 JVM 参数（JDK 8 语法）
      - JAVA_OPTS=-Xms512m -Xmx1024m -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=/app/logs/
    deploy:
      resources:
        limits:
          memory: 1G
...
spring:
  ...
  # ---------- Redis（Spring Boot 2.5.x 前缀：spring.redis）----------
  redis:
...
完整配置请查看 `src/main/resources/application-prod.yml`，其中还包含 Druid 连接池调优、
Actuator 端点暴露控制、Tomcat 线程池等生产级配置。

---

## 15.5.x Spring Boot 2.5 配置与环境变量对照表

| Spring Boot 2.5 配置项 | 对应环境变量 | 说明 |
| ----------------------- | ------------ | ---- |
| `spring.datasource.url` | `SPRING_DATASOURCE_URL` | 数据库连接 |
| `spring.datasource.username` | `SPRING_DATASOURCE_USERNAME` | 数据库用户 |
| `spring.datasource.password` | `SPRING_DATASOURCE_PASSWORD` | 数据库密码 |
| `spring.redis.host` | `SPRING_REDIS_HOST` | Redis 主机（2.5 前缀！） |
| `spring.redis.port` | `SPRING_REDIS_PORT` | Redis 端口 |
| `spring.redis.password` | `SPRING_REDIS_PASSWORD` | Redis 密码 |
| `spring.profiles.active` | `SPRING_PROFILES_ACTIVE` | 激活的 Profile |
...
**场景 4：Redis 连接失败（Spring Boot 2.5 特有排查点）**
...
```bash
# 1. 确认环境变量前缀是 SPRING_REDIS_*（2.5.x），不是 SPRING_DATA_REDIS_*（3.x）
docker exec blog-app sh -c 'echo $SPRING_REDIS_HOST'
# 应输出：blog-redis

# 2. 如果误用了 SPRING_DATA_REDIS_HOST，2.5.12 会忽略该变量，
#    应用回退到 application-prod.yml 中的默认值 localhost，导致连接失败
```
...
| 组件 | 技术 | 端口 | 说明 |
|------|------|------|------|
| 博客应用 | Spring Boot 2.5.12 + JDK 8 | 8080 | 多阶段构建，非 root 运行，eclipse-temurin:8-jre |
| 数据库 | MySQL 8.0 | 3306 | utf8mb4，mysql_native_password 兼容旧驱动，自动初始化 |
| 缓存 | Redis 7 Alpine | 6379 | AOF 持久化，与 Spring Data Redis 2.5.x（Lettuce 6.1）兼容 |
| 反向代理 | Nginx 1.25 Alpine | 80/443 | Gzip 压缩，限流，静态资源缓存 |

**Spring Boot 2.5.12 部署适配要点回顾：**

| 适配点 | 说明 |
|--------|------|
| 基础镜像 | JDK 17 → **eclipse-temurin:8-jre** / maven:3.8-openjdk-8 |
| Redis 环境变量 | `SPRING_DATA_REDIS_*` → **`SPRING_REDIS_*`** |
| Redis 配置前缀 | `spring.data.redis.*` → **`spring.redis.*`** |
| JVM 参数 | G1GC（JDK 17 默认）→ Parallel GC（JDK 8 默认，可按需切换） |
| 分层 JAR | 2.3+ 支持 layertools，2.5.12 同样可用（本教程未默认启用） |
| MySQL 驱动 | 使用 `mysql:mysql-connector-java`（2.5.12 管理的旧坐标） |
...
```
15-docker-deploy/
├── Dockerfile                          # 多阶段构建 Dockerfile（JDK 8）
...
└── src/main/resources/
    └── application-prod.yml            # 生产环境 Spring Boot 配置（2.5.x 前缀）
```
